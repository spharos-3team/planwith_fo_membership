...
class DeployControllerIntegrationTests {
	...
	void deployCheckReturnsMarker() throws Exception {
		mockMvc.perform(get("/api/planwith-fo-membership/deploy-check"))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.service").value("planwith-fo-membership"))
				.andExpect(jsonPath("$.marker").value("planwith-fo-membership-deploy-v1"))
				.andExpect(jsonPath("$.message").value("planwith-fo-membership deploy pipeline ok"));
	}

	@Test
	void loginSucceedsWithConfiguredCredentials() throws Exception {
		mockMvc.perform(post("/api/planwith-fo-membership/login")
